...
export const getDaftarbyId = async (req, res) => {
    ...
    try {
        const response = await Instansi.findByPk(instansiId, {
            include: Surat
        })
        if (response) {
            const dataSurat = response.Surat;
            const dataPelamar = await Pelamar.findAll({
                where: {
            ...
            // Jika Instansi dengan ID tersebut tidak ditemukan, kirimkan pesan error
